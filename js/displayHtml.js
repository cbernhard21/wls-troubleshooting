...
export async function displayOverdriveHtml() {
    ...
    let allOverdriveHtml = overdriveData
        .map((item) => {
            let stepsHtml = '';
    ...
    let overdriveIpHtml = overdriveIps
        .map((item) => {
...
export async function displayVintenHtml() {
    ...
    //variables
    const vintenContainer = document.querySelector('.data-container');
    const vintenData = data.services.vinten;

    //loop to create each issue section
    let allVintenDataHtml = vintenData
        .map((item) => {
            let stepsHtml = '';
            let notes = item.notes;
            let stepsArr = item.steps;
       ...
       `;
        ...
        .join('');
    vintenContainer.innerHTML = allVintenDataHtml;
}

export async function displaySwitchHtml() {
    console.log('Hello from switch PCRs');
}